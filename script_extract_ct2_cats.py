...
def scsv_dump_file(tool, language):
    return Path(__file__).parent / f"ws-static/data/{language}/scsv_webdump/{tool.lower()}.csv"

def scsv_reference_file(tool):
    return scsv_dump_file(tool, "en")

def category_table_output(tool, language):
    return Path(__file__).parent / f"ws-static/categories_{tool.lower()}_{language}.csv"
