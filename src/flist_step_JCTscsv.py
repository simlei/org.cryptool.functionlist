...
def jctrecord(df: pandas.DataFrame, currentFrame: list, inputfile: Path, currentLine: int):
    """
    appends a record (obtained from JCT dynamic csv) to a dataframe and returns it.
    assigns ids for rows based on the line number and prefixes with 'JCT:dynamic'
    """
    ...
    for (entrySplit,lineNr) in entriesSplit:
        ...
        how_implemented = f"JCT:{howimpl_payload}"
        pathelements = entrySplit[2].split("\\ ")
        path = " \\ ".join([how_implemented] + pathelements)
        category = "0) TODO: not inferred for dynamic JCT output"
        id = f"JCT:dynamic:{flist.makeId(functionality, path, howimpl_payload)}"
        record_dict = {
...
def CreateJCTSCSV(inputfile: Path, outputfile: Path):
